...
## 1. Firebase SQL Connect (Cloud SQL Postgres)
*Status: Active*
...
**Architecture Note for Mentoring Feature:**
Mentoring requests use a role-reversal trick to avoid schema changes.
- A Mentoree applies for mentoring by creating a HelpRequest (with category='MENTORING') but assigning equesterId to the targeted Mentor.
- The Mentoree then immediately creates an Application on that HelpRequest.
- This causes the Mentoring Request to appear in the Mentor's "Applications" tab as a service they posted, allowing them to Accept/Decline and naturally start a chat!
...
**Currently Stored Here:**
*   **conversations/{conversationId}/messages**: The actual chat text exchanged between users. We migrated this here to bypass SQL Connect quotas and make chat instant.
...
*   **Google OAuth:** Implemented using a same-origin reverse proxy via Vercel (vercel.json rewrites /__/auth) to bypass strict third-party cookie blocking (e.g., Opera GX, Safari).
*   **Session State:** Controlled by Firebase Auth, but full user registration is contingent on the corresponding SQL Connect User record existing.
